...
# ==========================================================
# 📋 Listar jobs de processamento de PDVs
# ==========================================================
...
def progresso_job(request: Request, job_id: str):
    """
    Retorna o status e progresso atual de um job em execução ou concluído.
    O campo 'meta' é atualizado pelo worker durante o processamento.
    """
    try:
        conn_redis = Redis(host="redis", port=6379)
        job = Job.fetch(job_id, connection=conn_redis)
        ...
        etapa = meta.get("step", "Aguardando atualização")
...
# ==========================================================
# 📥 Download de registros inválidos de um job
# ==========================================================
from fastapi.responses import FileResponse

@router.get("/jobs/{job_id}/download-invalidos", dependencies=[Depends(verify_token)], tags=["Jobs"])
def download_invalidos(request: Request, job_id: str):
    """
    Retorna o arquivo CSV de registros inválidos (caso exista)
    associado ao job informado.
    O caminho do arquivo é buscado no banco (historico_pdv_jobs.arquivo_invalidos).
    """
    user = request.state.user
    tenant_id = user["tenant_id"]
    ...
    try:
        ...
        if not row or not row[0]:
            raise HTTPException(status_code=404, detail="Nenhum arquivo de inválidos encontrado para este job.")

        caminho_arquivo = row[0]
        if not os.path.exists(caminho_arquivo):
            raise HTTPException(status_code=404, detail=f"Arquivo não encontrado: {caminho_arquivo}")

        logger.info(f"📥 Download solicitado para arquivo de inválidos: {caminho_arquivo}")
        return FileResponse(
            caminho_arquivo,
            media_type="text/csv",
        ...
        logger.error(f"❌ Erro ao baixar arquivo de inválidos para job {job_id}: {e}", exc_info=True)
