...
def _angulo_polar(lat, lon, lat_ref, lon_ref):
    """Calcula o ângulo polar (em radianos) em relação ao ponto de referência."""
    dlon = radians(lon - lon_ref)
    y = sin(dlon) * cos(radians(lat))
    x = (
        cos(radians(lat_ref)) * sin(radians(lat))
        - sin(radians(lat_ref)) * cos(radians(lat)) * cos(dlon)
    )
    ang = atan2(y, x)
...
def capacitated_sweep(pdvs, max_capacity=100, random_state=42):
    ...
    # ==========================================
    # 7️⃣ Geração correta dos labels (FIX FINAL)
    # ==========================================
    ...
    for cid, cluster in enumerate(clusters):
        for p in cluster:
            # descobre a posição de p dentro da lista pdvs_ord
            idx_ord = pdvs_ord.index(p)

            # converte para índice real no vetor original pdvs
            idx_real = ordem[idx_ord]
    ...
    for cid, cluster in enumerate(clusters):
        arr = np.array([[p.lat, p.lon] for p in cluster])
        centro = centers[cid]
        d = [_haversine_km(p.lat, p.lon, centro[0], centro[1]) for p in cluster]
        dist_medias.append(np.mean(d))

    media_intra = float(np.mean(dist_medias)) if dist_medias else 0.0

    # 9️⃣ Log final
    logger.success(
        f"✅ Capacitated Sweep concluído | K={len(clusters)} | "
        f"média={np.mean(tamanhos):.1f} PDVs | dist_média_intra={media_intra:.2f} km"
    )
