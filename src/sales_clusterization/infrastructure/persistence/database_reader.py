...
def carregar_pdvs(tenant_id: int, uf: Optional[str] = None, cidade: Optional[str] = None) -> List[PDV]:
    ...
    - Remove coordenadas inválidas (NaN, inf, zeros falsos, duplicadas).
    - Retorna lista de objetos PDV prontos para clusterização.
    """
    base_query = """
    ...
    # ------------------------------------------------------------------
    # 🔍 Limpeza e validação das coordenadas
    # ------------------------------------------------------------------
    pdvs_limp = []
    invalidos = 0
    coords_vistos = set()
    ...
    for row in rows:
        try:
            ...
            # Evita duplicatas exatas (mesmas coordenadas)
    ...
    logger.info(
        ...
        f"🧹 {invalidos} registros inválidos removidos"
    )
...
def get_cidades_por_uf(tenant_id: int, uf: str) -> list[str]:
    ...
    Usado apenas para logs ou estatísticas; não é mais iterado quando cidade=None.
    """
    query = """
